...
router.delete("/", auth, async (req, res) => {
  try {
    ...
    const kittensToDelete = await Kitten.find({ owner: req.user.id });

    await Adoption.deleteMany({ kitten: { $in: kittensToDelete } });

    await Kitten.deleteMany({ owner: req.user.id });
...
router.delete("/kittens/:kitten_id", auth, async (req, res) => {
  try {
    ...
    await Adoption.findOneAndRemove({ kitten: req.params.kitten_id });
